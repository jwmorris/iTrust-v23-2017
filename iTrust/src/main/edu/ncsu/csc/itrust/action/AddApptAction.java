...
public class AddApptAction extends ApptAction {
	...
	public String addAppt(ApptBean appt, boolean ignoreConflicts) throws FormValidationException, SQLException, DBException {
		...
		try {
			...
			TransactionLogger.getInstance( factory ).logTransaction(TransactionType.APPOINTMENT_ADD, loggedInMID, appt.getPatient(), "");
			if(ignoreConflicts){
				TransactionLogger.getInstance( factory ).logTransaction(TransactionType.APPOINTMENT_CONFLICT_OVERRIDE, loggedInMID, appt.getPatient(), "");
			}
